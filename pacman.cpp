...
void user_input()
{
	...
	{		
		...
		if(c1 == KEY_UP || c1 == KEY_DOWN || c1 == KEY_LEFT || c1 == KEY_RIGHT){
			myPacMan.vx = 0;
			myPacMan.vy = 0;
			
			switch(c1){
				case KEY_UP:
					myPacMan.vy = -1; break; // cursor up
					break;
				case KEY_DOWN:
					myPacMan.vy = +1; break; // cursor down
					break;
				case KEY_LEFT:
					myPacMan.vx = -1; break; // cursor left
					break;
				case KEY_RIGHT:
					myPacMan.vx = +1; break; // cursor right 
					break;
			}
		}
		/*if (c1 == -32)
...
void show_playfield()
{
	...
	for (int i = 0; i < H; i++)
	{
		for (int j = 0; j < W; j++)
		{
			...
			{
				wprintw(wui, "%c", ghostField[i][j]);
				wrefresh(wui);
			}
			else if (playerField[i][j] == PACMAN_SYMBOL) 
			{
				wprintw(wui, "%c", playerField[i][j]);
				wrefresh(wui);
			}
			else 
			{
				wprintw(wui, "%c", playfield[i][j]);
				wrefresh(wui);
			}
		}
		wprintw(wui, "\n");
	}
	wprintw(wui, "Score:%3d - Remaining:%3d - Lives:%2d\n", myPacMan.food_collected, foodToWin - myPacMan.food_collected, myPacMan.lives);
	wrefresh(wui);
}
...
void check_collisions()
{
	...
	{
		set_cursor_position(H / 2 - 3, W / 2 - 9);
		wprintw(wui, "   GAME START   ");
		set_cursor_position(H / 2 - 2, W / 2 - 8);
		myPacMan.lives--;
		wprintw(wui, "   Lifes:%d   ", myPacMan.lives );
		set_cursor_position(H / 2 - 1, W / 2 - 8);
		wprintw(wui, "   Score:%d   ", myPacMan.food_collected);
...
int main()
{
	...
	*/
	...
	set_cursor_position(H / 2 - 3, W / 2 - 6);
	wprintw(wui, "   PAC-MAN   ");
	set_cursor_position(H / 2 - 1, W / 2 - 6);
	wprintw(wui, " Press Any Key   ");
	wrefresh(wui);
	wgetch(wui);
	...
	while (!game_over)
	{
		...
		if (myPacMan.food_collected >= foodToWin)
		{
			set_cursor_position(H / 2 - 3, W / 2 - 9);
			wprintw(wui, "                ");
			set_cursor_position(H / 2 - 2, W / 2 - 7);
			wprintw(wui, "   YOU WON!   ");
			set_cursor_position(H / 2 - 1, W / 2 - 8);
			printw("                ");
			wrefresh(wui);
